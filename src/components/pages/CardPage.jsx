...
import { clearCartAC, plusPizzaItemAC,minusPizzaItemAC } from "../../redux/cartReducer";
import EmptyCartInfo from "../EmptyCartInfo";
import PizzaInCart from "./PizzaInCart";
import { removePizzaItemFromCartAC } from "../../redux/cartReducer";
...
const CardPage = () => {
  ...
  const removePizzaItem = (id) => {
    dispatch(removePizzaItemFromCartAC(id))
  }
  ...
  const onClearCart = () => {
    if (window.confirm('Are you sure want to delete all pizzas in cart?')){
  ...
  const onPlusItem = (id) => {
    dispatch(plusPizzaItemAC(id))
  }

  const onMinusItem = (id) => {
    dispatch(minusPizzaItemAC(id))
  }
    ...
    <path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                ...
                size = {e.size} 
                img = {e.imageUrl}
                totalPrice ={items[e.id].totalPrice}
                totalCount ={items[e.id].items.length}
